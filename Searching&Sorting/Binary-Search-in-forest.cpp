//There are n trees in a forest. The heights of the trees is stored in array tree[], where tree[i] denotes the height of the ith tree in the forest. 
//If the ith tree is cut at a height H, then the wood collected is tree[i] - H, provided tree[i] > H. If the total woods that needs to be collected is exactly equal to k,
//find the height H at which every tree should be cut (all trees have to be cut at the same height). If it is not possible then return -1 else return H.

//soln:-
  int ispossible(int tree[], int n,int mid)
   {
       
       int total=0;
       for(int i=0;i<n;i++)
       {
           if(tree[i]>mid)
           {
               total+=tree[i]-mid;
           }
       }
       return total;
   }
    int find_height(int tree[], int n, int k) {
        // code here
        
        int low=0;
        int high=INT_MIN;
        for(int i=0;i<n;i++)
        {
            if(tree[i]>high)
            {
                high=tree[i];
            }
        }
        
        while(low<=high)
        {
            int mid=low+(high-low)/2;
            int total=ispossible(tree,n,mid);
            if(total==k)
            {
                return mid;
            }
            else if(total<k)
            {
                high=mid-1;
                
            }
            else{
                low=mid+1;
            }
            
        }
        
        return -1;
        
    }
